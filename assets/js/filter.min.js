...
(function ($) {
    ...
    function dispatchFilteredPaginationRequest(payload, interactionOptions) {
        ...
        if (isAppendMode) {
            const maxPages = parsePageNumber(filterState.totalPages);
            const currentPage = parsePageNumber(filterState.currentPage) || 1;
            if (maxPages && currentPage >= maxPages) {
                const wrapper = interactionOptions && (interactionOptions.wrapper || interactionOptions.trigger) ? interactionOptions.wrapper || interactionOptions.trigger : null;
                markLoadMoreExhausted(wrapper);
                enforceLoadMoreVisibility();
                return;
            }
            if (maxPages && targetPage > maxPages) {
                const wrapper = interactionOptions && (interactionOptions.wrapper || interactionOptions.trigger) ? interactionOptions.wrapper || interactionOptions.trigger : null;
                markLoadMoreExhausted(wrapper);
                enforceLoadMoreVisibility();
                return;
    ...
    function markLoadMoreExhausted($wrapper) {
        ...
        if (scrollAutoObserver && scrollAutoWatched.has($wrapper.get(0))) {
            try {
                scrollAutoObserver.unobserve($wrapper.get(0));
            } catch (_error) { }
            scrollAutoWatched.delete($wrapper.get(0));
        }
        if (scrollAutoFallbackTargets && scrollAutoFallbackTargets.length) {
            scrollAutoFallbackTargets = scrollAutoFallbackTargets.filter(function (el) {
                return el !== $wrapper.get(0);
            });
        }
    }
    ...
    function restoreLoadMoreWrapper($wrapper) {
        $wrapper = coerceTojQuery($wrapper);
        if (!$wrapper || !$wrapper.length) return;
        $wrapper.removeClass('dapfforwc-load-more-complete').attr('aria-disabled', 'false').show();
        const $button = findAutoScrollClickable($wrapper);
        if ($button && $button.length) {
        ...
        if ($wrapper.length && scrollAutoTriggeredPages && typeof scrollAutoTriggeredPages.delete === 'function') {
            scrollAutoTriggeredPages.delete($wrapper.get(0));
        }
    }
    ...
    function updateTotalPagesFromContext(context) {
        ...
        } else if (filterState.currentPage) {
            filterState.totalPages = parsePageNumber(filterState.currentPage) || 1;
        } else if (!filterState.totalPages) {
            filterState.totalPages = 1;
        }
        enforceLoadMoreVisibility();
    }
        ...
        filterState.currentRequest = $.ajax({
            ...
            error: function (xhr, status, error) {
                if (status !== 'abort') {
                    ...
                    if (xhr && xhr.status === 404 && paginationInteraction && paginationInteraction.mode === 'append') {
                        markLoadMoreExhausted(paginationInteraction.wrapper || paginationInteraction.trigger);
                        enforceLoadMoreVisibility();
                    }
                }
            },
    ...
    function renderAjaxHTML(response, currentUrl, url) {
        ...
        updateTotalPagesFromContext(tempDiv);
