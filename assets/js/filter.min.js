...
(function ($) {
    ...
    function getActivePriceRange() {
        const $form = $(dapfforwc_FILTER.formSelector);
        let $minRange = $form.find('.range-min');
        let $maxRange = $form.find('.range-max');

        if (!$minRange.length || !$maxRange.length) {
            $minRange = $form.find('input[name="mn_price"]');
            $maxRange = $form.find('input[name="mx_price"]');
        }

        if (!$minRange.length || !$maxRange.length) return null;

        const defaultMin = $minRange.attr('min') ?? '';
        const defaultMax = $maxRange.attr('max') ?? '';
        const currentMin = $minRange.val() ?? defaultMin;
        const currentMax = $maxRange.val() ?? defaultMax;
    ...
    function bindFilterEvents($form) {
        ...
        if (dapfforwc_styleoptions.apply_behavior.reset_btn !== "only_apply") {
    ...
    function handleFilterChange($form, $page = 1, $isCacheOnly = false) {
        ...
        if (!priceHasChanged && $minRange.length && $maxRange.length) {
