...
(function ($) {
    ...
    function loadFilteredContentAjax(url, formData, currentUrl) {
        ...
        filterState.currentRequest = $.ajax({
            ...
            success: function (response) {
                ...
                const productshtml = tempDiv.find(product_selector);
                const updatedform = tempDiv.find(dapfforwc_FILTER.formSelector);
                const paginationhtml = tempDiv.find(paginationSelector_shortcode ?? pagination_selector);
                const productCountHtml = tempDiv.find('.woocommerce-result-count');
                if (productshtml.length > 0) {
                    $(product_selector).html(productshtml.html());
                } else {
                    $(product_selector).html('<p class="woocommerce-info">No products were found matching your selection.</p>');
                    $(".woocommerce-ordering").remove();
                }
                if (paginationhtml && paginationhtml.length > 0) {
                    $(paginationSelector_shortcode ?? pagination_selector).html(paginationhtml.html());
                } else {
                    $(paginationSelector_shortcode ?? pagination_selector).empty();
                }
                ...
                if (productCountHtml.length > 0) {
                    $('.woocommerce-result-count').html(productCountHtml.html());
                } else {
    ...
    function applyFiltersFromUrl(filtersString) {
        ...
        prefixToAttribute[attrprefix.tag] = 'tags';
    ...
    function transformToSeoUrl(url, seoOptions) {
        ...
        if (searchParams.has('search') && searchParams.get('search') !== '') {
            seoParams.append('title', searchParams.get('search'));
        }
        ...
        for (const [key, value] of searchParams.entries()) {
            ...
            if (key.startsWith('pa_') && value !== '') {
                // Extract attribute name, e.g., 'pa_brand' -> 'brand'
