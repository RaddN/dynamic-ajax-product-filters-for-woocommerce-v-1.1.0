...
(function ($) {
    ...
    const dapfforwc_FILTER = {
        ...
        filterTriggers: 'input[type="checkbox"], input[type="radio"], select, input[type="range"], input[type="number"]',
    ...
    function toRelativeFilterUrl(url) {
        try {
            const normalizedUrl = new URL(url || window.location.href, window.location.origin);
    ...
    function syncHiddenRefererField($form = null, url = null) {
        const $targetForm = ($form && $form.length) ? $form : $(dapfforwc_FILTER.formSelector).first();
        if (!$targetForm.length) {
            return;
        }
        $targetForm.find('input[name="_wp_http_referer"]').val(toRelativeFilterUrl(url || window.location.href));
    }
    ...
    function getAdvancedCssSelectorSetting(key, fallback) {
        if (!advancesettings || !Object.prototype.hasOwnProperty.call(advancesettings, key)) {
            return fallback;
        }

        const raw = String(advancesettings[key] || '').trim();
        if (!raw) {
            return fallback;
        }
        ...
        try {
            ...
            if (typeof plugincydebugLog === 'function') {
                plugincydebugLog('Invalid advanced selector detected:', key, raw, error);
            }
            return fallback;
        }
    }

    dapfforwc_FILTER.sortingSelector = getAdvancedCssSelectorSetting('sorting_selector', dapfforwc_FILTER.sortingSelector);
    dapfforwc_FILTER.resultCountSelector = getAdvancedCssSelectorSetting('result_count_selector', dapfforwc_FILTER.resultCountSelector);

    function getAdjacentNavigationSelectors(baseSelector) {
        return normalizePaginationSelectorInput(baseSelector).map(function (selector) {
    ...
    function normalizeAdvancedPaginationMode(value) {
        const normalized = String(value || '')
            .trim()
            .toLowerCase()
            .replace(/[\s-]+/g, '_');

        if (!normalized || normalized === 'number' || normalized === 'numbers' || normalized === 'numeric') {
            return 'number';
        }

        if (normalized === 'number_prev_next' || normalized === 'numbers_prev_next' || normalized === 'number_and_prev_next' || normalized === 'number_prevnext') {
            return 'number_prev_next';
        }

        if (normalized === 'load_more' || normalized === 'loadmore') {
            return 'load_more';
        }

        if (normalized === 'infinite_scroll' || normalized === 'infinitescroll') {
    ...
    function getAdvancedPaginationSetting(key) {
        if (!advancesettings || !Object.prototype.hasOwnProperty.call(advancesettings, key)) {
            return '';
        }
        return String(advancesettings[key] || '').trim();
    }

    const ADVANCED_PAGINATION_SETTINGS = {
        enabled: getAdvancedPaginationSetting('advanced_pagination_enabled') === 'on',
        mode: normalizeAdvancedPaginationMode(getAdvancedPaginationSetting('advanced_pagination_mode')),
        prevSelectors: normalizePaginationSelectorInput(getAdvancedPaginationSetting('advanced_pagination_prev_selector')),
        nextSelectors: normalizePaginationSelectorInput(getAdvancedPaginationSetting('advanced_pagination_next_selector')),
        loadMoreSelectors: normalizePaginationSelectorInput(getAdvancedPaginationSetting('advanced_pagination_load_more_selector')),
        infiniteScrollSelectors: normalizePaginationSelectorInput(getAdvancedPaginationSetting('advanced_pagination_infinite_scroll_selector'))
    };
    ...
    function tokenizeHintValue(value) {
    ...
    function tokensContainHint(tokens, hints) {
        if (!tokens || !tokens.length) return false;
        return hints.some(function (hint) {
            return tokens.indexOf(String(hint).toLowerCase()) !== -1;
        });
    }
    ...
    function elementHasDataHint(element, hints) {
        ...
        for (const key in dataset) {
            ...
            const keyTokens = tokenizeHintValue(key);
            const valueTokens = tokenizeHintValue(dataset[key]);
            if (tokensContainHint(keyTokens, hints) || tokensContainHint(valueTokens, hints)) {
                return true;
            }
        }
        return false;
    ...
    function isLoadMoreTrigger($element, $wrapper, detail) {
        ...
        if (ADVANCED_PAGINATION_SETTINGS.enabled) {
            if (ADVANCED_PAGINATION_SETTINGS.mode === 'load_more') {
                if (ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors.length) {
                    return elementMatchesConfiguredSelectors($element, ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors) ||
                        elementMatchesConfiguredSelectors($wrapper, ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors);
                }
                return !!(($wrapper && $wrapper.length) || ($element && $element.length));
            }

            if (ADVANCED_PAGINATION_SETTINGS.mode === 'infinite_scroll') {
                if (ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors.length) {
                    return elementMatchesConfiguredSelectors($element, ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors) ||
                        elementMatchesConfiguredSelectors($wrapper, ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors);
                }
                return !!(($wrapper && $wrapper.length) || ($element && $element.length));
            }
    ...
    function elementMatchesConfiguredSelectors($element, selectors) {
        if (!$element || !$element.length) return false;
        const selectorList = Array.isArray(selectors) ? selectors : normalizePaginationSelectorInput(selectors);
        if (!selectorList.length) return false;

        return selectorList.some(function (selector) {
            try {
                return $element.is(selector) || $element.closest(selector).length > 0 || $element.find(selector).length > 0;
            } catch (_error) {
                return false;
            }
        });
    ...
    function findClosestConfiguredElement($element, selectors) {
    ...
    function findConfiguredElementWithinContext($context, selectors) {
    ...
    function getAdvancedPaginationTriggerSelectors(mode) {
        const targetMode = mode || ADVANCED_PAGINATION_SETTINGS.mode;
        if (targetMode === 'number_prev_next') {
            return ADVANCED_PAGINATION_SETTINGS.prevSelectors.concat(ADVANCED_PAGINATION_SETTINGS.nextSelectors);
        }
        if (targetMode === 'load_more') {
            return ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors.slice();
        }
        if (targetMode === 'infinite_scroll') {
            return ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors.slice();
        }
        return [];
    }
    ...
    function collectPaginationSelectors() {
        ...
        if (ADVANCED_PAGINATION_SETTINGS.enabled) {
            ...
            getAdjacentNavigationSelectors(dapfforwc_FILTER.resultCountSelector).forEach(function (selector) {
                fallbackSelectors.push(selector);
            });

            fallbackSelectors.forEach(sel => addSelector(sel));
        }
    ...
    function collectPaginationEventSelectors() {
        const selectors = new Set();
        const addSelector = (selector) => {
            if (!selector || typeof selector !== 'string') return;
            const trimmed = selector.trim();
            if (!trimmed || !isValidJquerySelector(trimmed)) return;
            selectors.add(trimmed);
        };
        ...
        getAdvancedPaginationTriggerSelectors().forEach(addSelector);
    ...
    function parsePageNumber(value, increment = 0) {
        ...
        if (typeof value === 'number') {
            ...
            const normalizedValue = String(value).replace(/(\d)[,\s\u00a0](?=\d{3}(?:\D|$))/g, '$1');
    ...
    function getPermalinkPrefixes() {
        return (dapfforwc_seo_permalinks_options && dapfforwc_seo_permalinks_options.dapfforwc_permalinks_prefix_options) || {};
    }

    function getConfiguredPermalinkKey(key, fallback) {
        const prefixes = getPermalinkPrefixes();
        if (prefixes && prefixes[key] && prefixes[key] !== '') {
            return prefixes[key];
        }
        return fallback;
    }

    function getPaginationQueryKey() {
        return getConfiguredPermalinkKey('pagination', 'paged');
    }

    function getOrderbyQueryKey() {
        return getConfiguredPermalinkKey('orderby', 'orderby');
    }

    function hasMeaningfulSerializedValue(value) {
    ...
    function shouldSerializeFormField(item) {
        if (!item || !item.name) {
            return false;
        }
        ...
        if (fieldName === 'gm-product-filter-nonce' || fieldName === '_wp_http_referer') {
            return true;
        }

        const paginationKeys = new Set([getPaginationQueryKey(), 'paged', 'product-page', 'page'].filter(Boolean));
        if (paginationKeys.has(fieldName)) {
            return false;
        }

        if ((fieldName === 'orderby' || fieldName === getOrderbyQueryKey()) && !hasMeaningfulSerializedValue(item.value)) {
            return false;
        }

        return hasMeaningfulSerializedValue(item.value);
    }

    function getExplicitOrderbyValue($form) {
        if (!$form || !$form.length) return '';
        const rawValue = (($form.find('#filter-orderby').val()) ?? $(dapfforwc_FILTER.sortingSelector).first().val() ?? '').toString().trim();
        if (!rawValue || rawValue === 'menu_order') {
            return '';
        }
        return rawValue;
    }

    function setExplicitOrderby($form, sortValue) {
        if (!$form || !$form.length) return '';
        const normalizedValue = (sortValue ?? '').toString().trim();
        let $orderbyInput = $form.find('#filter-orderby');

        if (!normalizedValue || normalizedValue === 'menu_order') {
        ...
        if ($orderbyInput.length === 0) {
            ...
            $form.append($orderbyInput);
        } else {
            $orderbyInput.val(normalizedValue);
        }

        return normalizedValue;
    }

    function resetSortingControlsToDefault() {
        try {
            $(dapfforwc_FILTER.sortingSelector).each(function () {
                const $sortingControl = $(this);
                if (!$sortingControl.length) {
                    return;
                }

                const fallbackValue = (($sortingControl.find('option').first().val()) ?? '').toString();
                const defaultValue = $sortingControl.find('option[value="menu_order"]').length ? 'menu_order' : fallbackValue;
                $sortingControl.val(defaultValue);
            });
        } catch (_error) {
            // Ignore sorting reset errors from third-party template markup.
        }
    }

    function syncSortingStateFromUrl(urlParams) {
        const $form = $(dapfforwc_FILTER.formSelector);
        if (!$form.length || !(urlParams instanceof URLSearchParams)) {
            return '';
        }

        const orderbyKey = getOrderbyQueryKey();
        const sortValue = ((urlParams.get(orderbyKey) ?? urlParams.get('orderby') ?? '') + '').trim();
        const explicitSortValue = setExplicitOrderby($form, sortValue);
        ...
        try {
            $(dapfforwc_FILTER.sortingSelector).each(function () {
                const $sortingControl = $(this);
                if ($sortingControl.find(`option[value="${explicitSortValue}"]`).length) {
                    $sortingControl.val(explicitSortValue);
                }
            });
        } catch (_error) {
    ...
    function syncPaginationStateFromUrl(urlParams) {
        if (!(urlParams instanceof URLSearchParams)) {
            return null;
        }

        const paginationKey = getPaginationQueryKey();
        const pageFromUrl = parsePageNumber(urlParams.get(paginationKey))
    ...
    function extractPageFromUrl(rawUrl) {
        ...
        const configuredPaginationKey = attrprefix && attrprefix.pagination && attrprefix.pagination !== '' ? attrprefix.pagination : null;
        const queryKeys = Array.from(new Set([configuredPaginationKey, 'page', 'paged', 'product-page'].filter(Boolean)));
        try {
            ...
            for (const key of queryKeys) {
                ...
                const fallbackQuery = rawUrl.match(new RegExp(`[?&]${escapedKey}=(\\d+)`, 'i'));
    ...
    function getCurrentPaginationPage($wrapper) {
        if ($wrapper && $wrapper.length) {
            const $current = $wrapper.find('.current, .is-active, .active, .numeric-current-page, [aria-current="page"], [aria-current="true"]').last();
            const currentPage = parsePageNumber($current.text());
            if (currentPage) return currentPage;
        }
        return parsePageNumber(filterState.currentPage);
    }

    function inferDirectionalPaginationPage($element, $wrapper) {
        ...
        const tokens = tokenizeHintValue(directionSource);
        if (!tokens.length) return null;

        const currentPage = getCurrentPaginationPage($wrapper);
        if (!currentPage) return null;

        if (tokens.indexOf('prev') !== -1 || tokens.indexOf('previous') !== -1) {
    ...
    function getPageFromElement($element, $wrapper) {
        if (ADVANCED_PAGINATION_SETTINGS.enabled && ADVANCED_PAGINATION_SETTINGS.mode === 'number_prev_next') {
            const currentPage = getCurrentPaginationPage($wrapper) || parsePageNumber(filterState.currentPage) || 1;
            if (elementMatchesConfiguredSelectors($element, ADVANCED_PAGINATION_SETTINGS.prevSelectors)) {
                return Math.max(1, currentPage - 1);
            }
            if (elementMatchesConfiguredSelectors($element, ADVANCED_PAGINATION_SETTINGS.nextSelectors)) {
                return currentPage + 1;
            }
        }
        ...
        if (rawPageToken) {
            ...
            if (normalizedToken === 'prev' || normalizedToken === 'previous' || normalizedToken === 'next') {
                const basePage = getCurrentPaginationPage($wrapper);
                if (basePage) {
        ...
        const directionalPage = inferDirectionalPaginationPage($element, $wrapper);
        if (directionalPage) return directionalPage;

        if ($wrapper && $wrapper.length) {
            const fromCurrent = parsePageNumber(getCurrentPaginationPage($wrapper), 1);
    ...
    const paginationEventSelectors = collectPaginationEventSelectors();
    ...
    function resolveAdvancedPaginationContext($target) {
        if (!ADVANCED_PAGINATION_SETTINGS.enabled || !$target || !$target.length) {
            return null;
        }

        let $clickable = $();

        if (ADVANCED_PAGINATION_SETTINGS.mode === 'number_prev_next') {
            $clickable = findClosestConfiguredElement($target, ADVANCED_PAGINATION_SETTINGS.prevSelectors);
            if (!$clickable.length) {
                $clickable = findClosestConfiguredElement($target, ADVANCED_PAGINATION_SETTINGS.nextSelectors);
            }
        } else if (ADVANCED_PAGINATION_SETTINGS.mode === 'load_more') {
            $clickable = findClosestConfiguredElement($target, ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors);
        } else if (ADVANCED_PAGINATION_SETTINGS.mode === 'infinite_scroll') {
            $clickable = findClosestConfiguredElement($target, ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors);
        }
        ...
        let $wrapper = pagination_selector ? $clickable.closest(pagination_selector) : $();
        if (!$wrapper.length && pagination_selector) {
            try {
                const $fallbackWrapper = $(pagination_selector).first();
    ...
    function getPaginationDelegateSelector() {
        const selectorParts = [];

        if (pagination_selector) {
            selectorParts.push(pagination_selector);
            selectorParts.push(`${pagination_selector} ${PAGINATION_INTERACTIVE_TARGETS}`);
        }

        paginationEventSelectors.forEach(function (selector) {
            if (selector) {
                selectorParts.push(selector);
            }
        });

        return Array.from(new Set(selectorParts.filter(Boolean))).join(', ');
    }
    ...
    function gatherPaginationWrappers(context, includeLoadMoreHints = false) {
        ...
        if (includeLoadMoreHints && !ADVANCED_PAGINATION_SETTINGS.enabled) {
    ...
    function collectAutoScrollWrappers(context) {
        const nodes = new Set();
        gatherPaginationWrappers(context || document, true).forEach(function ($wrapper) {
            if ($wrapper && $wrapper.length) {
                nodes.add($wrapper.get(0));
            }
        });

        if (ADVANCED_PAGINATION_SETTINGS.enabled && ADVANCED_PAGINATION_SETTINGS.mode === 'infinite_scroll' && ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors.length) {
            ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors.forEach(function (selector) {
                const $found = querySelectorsInContext(context || document, selector);
                $found.each(function () {
                    nodes.add(this);
                });
            });
        ...
        return Array.from(nodes).map(function (element) { return $(element); });
    }
    ...
    function shouldAutoScrollWrapper($wrapper) {
        ...
        if (ADVANCED_PAGINATION_SETTINGS.enabled) {
            if (ADVANCED_PAGINATION_SETTINGS.mode !== 'infinite_scroll') return false;
            if (!ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors.length) {
                return true;
            }
            if (elementMatchesConfiguredSelectors($wrapper, ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors)) return true;
            const $clickable = findAutoScrollClickable($wrapper);
            return !!($clickable && elementMatchesConfiguredSelectors($clickable, ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors));
        }
    ...
    function findAutoScrollClickable($wrapper) {
        if (!$wrapper || !$wrapper.length) return null;
        if (ADVANCED_PAGINATION_SETTINGS.enabled) {
            const configuredSelectors = ADVANCED_PAGINATION_SETTINGS.mode === 'load_more'
                ? ADVANCED_PAGINATION_SETTINGS.loadMoreSelectors
                : (ADVANCED_PAGINATION_SETTINGS.mode === 'infinite_scroll'
                    ? ADVANCED_PAGINATION_SETTINGS.infiniteScrollSelectors
                    : []);
            if (configuredSelectors.length) {
                const $configured = findConfiguredElementWithinContext($wrapper, configuredSelectors);
    ...
    function processPaginationClickEvent(e) {
        if (!pagination_selector && !paginationEventSelectors.length) return false;
        if (e && e.dapfforwcHandled) return true;
        const $target = $(e && e.target);
        if (!$target || !$target.length) return false;

        let $wrapper = pagination_selector ? $target.closest(pagination_selector) : $();
        let $clickable = $target.closest(PAGINATION_INTERACTIVE_TARGETS);
        const advancedContext = resolveAdvancedPaginationContext($target);

        if (advancedContext) {
            if (!$wrapper.length) {
                $wrapper = advancedContext.$wrapper;
            }
            $clickable = advancedContext.$clickable;
        }

        if (!$wrapper.length) return false;
        const isExplicitAdvancedTrigger = !!(advancedContext && advancedContext.$clickable && advancedContext.$clickable.length && $clickable && $clickable.length && advancedContext.$clickable.get(0) === $clickable.get(0));
        if (!$clickable.length || (!$clickable.closest($wrapper).length && !isExplicitAdvancedTrigger)) {
            $clickable = isExplicitAdvancedTrigger ? advancedContext.$clickable : $wrapper;
        }
    ...
    function attachPaginationEvents() {
        const delegateSelector = getPaginationDelegateSelector();
        if (!delegateSelector) return;
    ...
    syncSortingStateFromUrl(urlParams);
    syncPaginationStateFromUrl(urlParams);
    if (urlParams.has('filters')) {
        ...
        const $form = $(dapfforwc_FILTER.formSelector);
        handleFilterChange($form, filterState.currentPage || 1, true);
    }
    /**
    ...
    function getSerializableFormArray($form) {
        const $root = ($form && $form.length) ? $form : $(dapfforwc_FILTER.formSelector);
        return $root.find(':input').filter(function () {
            return $(this).closest('.default_values').length === 0;
        }).serializeArray();
    }

    function store_selected_values() {
        const filtersByType = {};
        const options_selector = "#product-filter input:checked";
        // Handle checked checkboxes and radios
        $(options_selector).filter(function () {
            return $(this).closest('.default_values').length === 0;
        }).each(function () {
    ...
    function getFilterField($scope, name) {
        const $root = ($scope && $scope.length) ? $scope : $(dapfforwc_FILTER.formSelector);
        const $fields = $root.find(`[name="${name}"]`);
        const $preferred = $fields.filter(function () {
            const type = ((this.type || '') + '').toLowerCase();
            return type !== 'checkbox' && type !== 'radio';
        }).first();
        return $preferred.length ? $preferred : $fields.first();
    }

    function getFilterFieldValue($scope, name) {
        const $field = getFilterField($scope, name);
    ...
    function getActiveRangeFilter(minName, maxName, $scope = null) {
        const $form = ($scope && $scope.length) ? $scope : $(dapfforwc_FILTER.formSelector);
        const $minInput = $form.find(`[name="${minName}"]`);
        const $maxInput = $form.find(`[name="${maxName}"]`);
        if (!$minInput.length && !$maxInput.length) return null;

        const defaultMin = (($minInput.data('default') ?? $minInput.attr('min')) ?? '').toString();
        const defaultMax = (($maxInput.data('default') ?? $maxInput.attr('max')) ?? '').toString();
        const currentMin = ((($minInput.val && $minInput.val()) ?? '') + '').trim();
        const currentMax = ((($maxInput.val && $maxInput.val()) ?? '') + '').trim();

        if (!currentMin && !currentMax) return null;
        const minIsDefault = currentMin === '' || String(currentMin) === String(defaultMin);
        const maxIsDefault = currentMax === '' || String(currentMax) === String(defaultMax);
    ...
    function filterOutDefaultDimensionRanges(formArray, $form = null) {
        const $targetForm = ($form && $form.length) ? $form : $(dapfforwc_FILTER.formSelector);
        ...
        dimensionRanges.forEach(([minName, maxName]) => {
            const $minInput = $targetForm.find(`[name="${minName}"]`);
            const $maxInput = $targetForm.find(`[name="${maxName}"]`);
            if (!$minInput.length && !$maxInput.length) {
                return;
            }

            const range = getActiveRangeFilter(minName, maxName, $targetForm);

            if (!range) {
                formArray = formArray.filter(function (item) {
                    return item.name !== minName && item.name !== maxName;
                });
            }
        });
    ...
    function resetRangeFilterToDefaults(minName, maxName) {
        ...
        if ($minInput.length) {
            const minResetValue = (($minInput.attr('type') || '').toLowerCase() === 'hidden') ? defaultMin : '';
            $minInput.val(minResetValue).trigger('change');
            syncDisplay($minInput, '.min-value', minResetValue === '' ? defaultMin : minResetValue);
        }
        if ($maxInput.length) {
            const maxResetValue = (($maxInput.attr('type') || '').toLowerCase() === 'hidden') ? defaultMax : '';
            $maxInput.val(maxResetValue).trigger('change');
            syncDisplay($maxInput, '.max-value', maxResetValue === '' ? defaultMax : maxResetValue);
        }
    }
    ...
    function selectedFilterShowProductTop() {
       ...
       <label style="font-size:12px;margin-left:5px;">x</label>`;
        ...
        fieldValues.forEach(({ name, key, cls }) => {
            const val = getFilterFieldValue($form, name);
            if (!val) return;
        ...
        activeRanges.forEach(({ key, min, max, cls, range }) => {
            if (!range) return;
            const prefix = getLabelPrefix(key, key);
            const rawValue = `${range.min || ''}-${range.max || ''}`;
            let displayValue = rawValue;
            if (range.min && !range.max) {
                displayValue = `Min ${range.min}`;
            } else if (!range.min && range.max) {
                displayValue = `Max ${range.max}`;
            }
            add(key, `selected_${min}_${max}`, `${prefix}: ${displayValue}`, rawValue, cls);
        });
    ...
    function bindFilterEvents($form) {
        ...
        $(document).on('click', '.reset-value', function () {
            ...
            $filterGroup.find('.items input[type="text"], .items input[type="search"], .items input[type="date"]').each(function () {
                $(this).val('');
            });
            if ($filterGroup.is('#dimensions, .dimensions')) {
        ...
        $form.on('click', '.dapfforwc-reset-filters-btn', function (e) {
            ...
            // Reset all text, search, and date inputs
            $form.find('input[type="text"], input[type="search"], input[type="date"]').val('');
            ...
            // Clear explicit sorting state stored for AJAX URLs and product ordering.
            setExplicitOrderby($form, '');
            resetSortingControlsToDefault();

            // Clear filter state
            filterState.pendingChanges = true;

            syncHiddenRefererField($form, baseUrl);
    ...
    $('.rfilterselected').on('click', 'li', function (e) {
        ...
        if ($item.hasClass('checked-plugincy-search-field')) {
            ...
            getFilterField($form, 'sku').val('').trigger('change');
            changeEventTriggered = true;
        } else if ($item.hasClass('checked-discount-field')) {
            getFilterField($form, 'discount').val('').trigger('change');
            changeEventTriggered = true;
        } else if ($item.hasClass('checked-date-filter-field')) {
            getFilterField($form, 'date_filter').val('').trigger('change');
            changeEventTriggered = true;
        } else {
    ...
    function handleFilterChange($form, $page = 1, $isCacheOnly = false) {
        ...
        // Serialize only meaningful filter values; pagination is appended separately.
        let formArray = getSerializableFormArray($form).filter(function (item) {
            return shouldSerializeFormField(item) && item.name !== '_wp_http_referer';
        });
        ...
        formArray = filterOutDefaultDimensionRanges(formArray, $form);
        ...
        if (priceHasChanged && default_min_price !== '' && default_max_price !== '') {
    ...
    function syncFormMetaFromAjaxResponse($form, $updatedForm) {
        if (!$form || !$form.length || !$updatedForm || !$updatedForm.length) {
            return;
        }

        const $currentDefaultValues = $form.children('.default_values').first();
        const $updatedDefaultValues = $updatedForm.children('.default_values').first();

        if ($updatedDefaultValues.length) {
            if ($currentDefaultValues.length) {
                $currentDefaultValues.replaceWith($updatedDefaultValues.clone());
            } else {
                $form.prepend($updatedDefaultValues.clone());
            }
        } else if ($currentDefaultValues.length) {
            $currentDefaultValues.empty();
        }

        const $updatedNonce = $updatedForm.find('input[name="gm-product-filter-nonce"]').first();
        if ($updatedNonce.length) {
            $form.find('input[name="gm-product-filter-nonce"]').val($updatedNonce.val());
        }
    }
    ...
    function renderAjaxHTML(response, currentUrl, url) {
        ...
        const paginationPrefix = (attrprefix && attrprefix.pagination && attrprefix.pagination !== '') ? attrprefix.pagination : 'paged';
        const orderbyPrefix = (attrprefix && attrprefix.orderby && attrprefix.orderby !== '') ? attrprefix.orderby : 'orderby';
        ...
        const explicitOrderby = getExplicitOrderbyValue($currentForm);
        const paginationUrlAttrs = ['href', 'data-url', 'data-href', 'data-link', 'data-target', 'data-page-url', 'data-next-url', 'data-load-url', 'data-pagination-url'];
        ...
        const appendFieldParam = (paramsObj, name, key) => {
            const val = getFilterFieldValue($currentForm, name);
            if (val) paramsObj.set(key && key !== '' ? key : name, val);
        };
        const buildRawVisibleQueryString = () => {
            let formArray = getSerializableFormArray($currentForm).filter(function (item) {
                return shouldSerializeFormField(item)
                    && item.name !== 'gm-product-filter-nonce'
                    && item.name !== '_wp_http_referer';
            });

            const $rangeMin = $currentForm.find('.range-min');
            const $rangeMax = $currentForm.find('.range-max');
            const $numberMin = $currentForm.find('input[name="mn_price"]');
            const $numberMax = $currentForm.find('input[name="mx_price"]');
            ...
            formArray = filterOutDefaultDimensionRanges(formArray, $currentForm);
            ...
            for (const key in merged) {
                ...
                mergedArray.push({
                    name: key,
                    value: uniqueValues.length > 1 ? uniqueValues.join(',') : uniqueValues[0]
                });
            }

            return $.param(mergedArray)
                .replace(/\+/g, '%20')
                .replace(/%5B/g, '')
                .replace(/%5D/g, '')
                .replace(/attribute/g, 'rplugpa_')
        ...
        const buildVisibleUrlForPage = (pageNumber) => {
            const normalizedPage = parsePageNumber(pageNumber) || 1;
            if (dapfforwc_seo_permalinks_options &&
                dapfforwc_seo_permalinks_options.use_attribute_type_in_permalinks === "on") {
                const rawQueryString = buildRawVisibleQueryString();
                let nextUrl = currentUrl + (rawQueryString ? '?' + rawQueryString : '');
                const seoParams = new URLSearchParams(rawQueryString);

                if (explicitOrderby && !seoParams.has('orderby')) {
                    seoParams.set('orderby', explicitOrderby);
                }

                if (normalizedPage > 1) {
                    seoParams.set('paged', String(normalizedPage));
                    seoParams.set('product-page', String(normalizedPage));
                    if (paginationPrefix && paginationPrefix !== 'paged' && paginationPrefix !== 'product-page') {
                        seoParams.set(paginationPrefix, String(normalizedPage));
                    }
                } else {
                    seoParams.delete('paged');
                    seoParams.delete('product-page');
                    if (paginationPrefix && paginationPrefix !== '') {
                        seoParams.delete(paginationPrefix);
                    }
                }

                const seoQueryString = seoParams.toString().replace(/%2C/g, ',');
                nextUrl = currentUrl + (seoQueryString ? '?' + seoQueryString : '');

                return transformToSeoUrl(nextUrl, dapfforwc_seo_permalinks_options).replace(/%2C/g, ',');
            }

            let selectedValuesByUser = store_selected_values();
            let filtersArray = [];

            for (let key in selectedValuesByUser) {
                if (Array.isArray(selectedValuesByUser[key])) {
                    filtersArray.push(...selectedValuesByUser[key]);
                }
            }
            ...
            if (explicitOrderby) {
                params.set(orderbyPrefix, explicitOrderby);
            }
            if (normalizedPage > 1) {
                params.set(paginationPrefix, normalizedPage);
            }

            let nextUrl = currentUrl;
            const paramString = params.toString();
            if (paramString) {
                nextUrl += (nextUrl.includes('?') ? '&' : '?') + paramString;
            }

            nextUrl = nextUrl.replace(/%2C/g, ',');

            return nextUrl;
        };
        const rewritePaginationLinks = ($container) => {
            if (!$container || !$container.length) {
                return;
            }

            $container.find(PAGINATION_INTERACTIVE_TARGETS).each(function () {
                const $item = $(this);
                const payload = buildPaginationPayload($item, $container);
                const targetPage = parsePageNumber(payload && payload.page);

                if (!targetPage) {
                    return;
                }

                const cleanUrl = buildVisibleUrlForPage(targetPage);
                if (!cleanUrl) {
                    return;
                }

                paginationUrlAttrs.forEach(function (attrName) {
                    if (attrName === 'href') {
                        if ($item.is('a') || $item.attr('href')) {
                            $item.attr('href', cleanUrl);
                        }
                        return;
                    }

                    if ($item.attr(attrName) !== undefined) {
                        $item.attr(attrName, cleanUrl);
                    }
                });
            });
        };
        if (dapfforwc_options.use_url_filter !== "ajax") {
            ...
            const historyUrl = buildVisibleUrlForPage(activePage);
            window.history.pushState({ dapfforwcFilter: true }, '', historyUrl);
        }
        ...
        syncFormMetaFromAjaxResponse($currentForm, $updatedForm);
        const $srcResultCount = tempDiv.find(dapfforwc_FILTER.resultCountSelector);
        const resultCountData = parseResultCountData($srcResultCount);
        const syncResolvedProductSelectorState = () => {
            window.getProductSelectorString = product_selector;
            window.getProductSelector = $(product_selector).length > 0;
            if (window.getProductSelector) {
                $('#dapfforwcpro-popup-notification, #dapfforwc-popup-notification').remove();
                $('#dapfforwcpro_issue_count, #dapfforwc_issue_count').hide().text('');
            }
        };
        const findNoProductsMounts = () => {
            const mounts = [];
            const seen = new Set();
            $('.bapf_no_products, .woocommerce-no-products-found').each(function () {
                const node = this;
                const text = ($(node).text() || '').trim();
                if (text && !/No products were found matching your selection\./i.test(text)) {
                    return;
                }
                const preferredNode = $(node).hasClass('bapf_no_products')
                    ? node
                    : ($(node).closest('.bapf_no_products').get(0) || node);
                if (preferredNode && !seen.has(preferredNode)) {
                    seen.add(preferredNode);
                    mounts.push(preferredNode);
                }
            });
            return $(mounts);
        };
        const restoreProductsIntoNoResultsState = ($sources) => {
            if (isAppendMode || !$sources.length) {
                return $();
            }
            const $mounts = findNoProductsMounts();
            if (!$mounts.length) {
                return $();
            }
            $mounts.each(function (i) {
                let $source = $sources.eq(i);
                if (!$source.length) {
                    $source = $sources.eq(0);
                }
                if ($source.length) {
                    $(this).replaceWith($source.clone());
                }
            });
            syncResolvedProductSelectorState();
        ...
        const nodeContainsSelector = (node, selector) => {
            if (!node || !selector) return false;
            try {
        ...
        const restoreMissingArchiveControls = ($targets, $sources) => {
            if (isAppendMode || !$targets || !$targets.length || !$sources || !$sources.length) {
                return;
            }
            ...
            $targets.each(function (i) {
                ...
                const sourceNode = $source.get(0);
                const sourceParent = sourceNode && sourceNode.parentElement;
                if (!sourceParent) {
                    return;
                }

                const sourceChildren = Array.from(sourceParent.children);
                const productIndex = sourceChildren.indexOf(sourceNode);
                if (productIndex < 0) {
                    return;
                }

                sourceChildren.slice(0, productIndex).forEach(function (node) {
                    const isNoticeWrapper = node.classList && node.classList.contains('woocommerce-notices-wrapper');
                    const isResultCount = nodeMatchesSelector(node, dapfforwc_FILTER.resultCountSelector);
                    const isSortingControl = nodeMatchesSelector(node, dapfforwc_FILTER.sortingSelector) || nodeContainsSelector(node, dapfforwc_FILTER.sortingSelector);

                    if (!isNoticeWrapper && !isResultCount && !isSortingControl) {
                        return;
                    }

                    if (isNoticeWrapper && $target.parent().children('.woocommerce-notices-wrapper').length) {
                        return;
                    }
                    if (isResultCount && $(dapfforwc_FILTER.resultCountSelector).length) {
                        return;
                    }
                    if (isSortingControl && $(dapfforwc_FILTER.sortingSelector).length) {
                        return;
                    }

                    $target.before($(node).clone());
                });
            });
        };

        // Update each .products by index
        let $targetsProducts = $(product_selector);
        ...
        } else if (!isAppendMode && $srcProducts.length) {
            $targetsProducts = restoreProductsIntoNoResultsState($srcProducts);
        }
        syncResolvedProductSelectorState();
        restoreMissingArchiveControls($targetsProducts, $srcProducts);
        ...
        const isPaginationContainerElement = function ($element) {
            if (!$element || !$element.length) {
                return false;
            }

            const nodeName = (($element.get(0) && $element.get(0).tagName) || '').toLowerCase();
            if (!['nav', 'div', 'section', 'ul', 'ol'].includes(nodeName)) {
                return false;
            }

            if ($element.parents('nav.woocommerce-pagination, .woocommerce-pagination, .plugincy-filter-pagination, .pagination, .wc-block-pagination, .wp-block-query-pagination, ul.page-numbers, ol.page-numbers').length) {
                return false;
            }

            return $element.find(PAGINATION_INTERACTIVE_TARGETS).length > 0
                || $element.is('ul.page-numbers, ol.page-numbers, nav.woocommerce-pagination, .woocommerce-pagination, .plugincy-filter-pagination, .pagination, .wc-block-pagination, .wp-block-query-pagination');
        };
        const filterPaginationContainers = function ($collection) {
            return $collection.filter(function () {
                return isPaginationContainerElement($(this));
            });
        };
        const insertedPaginationSources = new Set();
        const resolvePaginationAnchor = function ($source) {
            if (!$source || !$source.length || !$srcProducts.length) {
                return null;
            }
            ...
            $srcProducts.each(function (productIndex) {
                const relation = this.compareDocumentPosition(sourceNode);
                if (relation & Node.DOCUMENT_POSITION_FOLLOWING) {
                    previousIndex = productIndex;
                    return;
                }
                if (nextIndex === -1 && (relation & Node.DOCUMENT_POSITION_PRECEDING)) {
            ...
            if (previousIndex !== -1) {
                return { anchorIndex: previousIndex, insertAfter: true };
            }

            if (nextIndex !== -1) {
                return { anchorIndex: nextIndex, insertAfter: false };
            }

            return { anchorIndex: 0, insertAfter: true };
        };
        paginationTargetsList.forEach(function (selector) {
            ...
            let $sources;
            try {
                $targets = filterPaginationContainers($(selector));
                $sources = filterPaginationContainers(tempDiv.find(selector));
            } catch (_error) {
                return;
            }
            if (!$targets.length) {
                if (isAppendMode || !$sources.length || !$targetsProducts.length) {
                    return;
                }
                ...
                $sources.each(function (i) {
                    const $source = $(this);
                    const sourceNode = $source.get(0);
                    if (!sourceNode || insertedPaginationSources.has(sourceNode)) {
                        return;
                    }

                    const anchor = resolvePaginationAnchor($source) || {
                        anchorIndex: Math.min(i, Math.max($targetsProducts.length - 1, 0)),
                        insertAfter: true
                    };
                    const anchorIndex = Math.min(anchor.anchorIndex, Math.max($targetsProducts.length - 1, 0));
                    const $anchorTarget = $targetsProducts.eq(anchorIndex);
                    if (!$anchorTarget.length) {
                        return;
                    }

                    const $clone = $source.clone();
                    rewritePaginationLinks($clone);
                    if (anchor.insertAfter) {
                        $anchorTarget.after($clone);
                    } else {
                        $anchorTarget.before($clone);
                    }

                    insertedPaginationSources.add(sourceNode);
                });
                return;
            }
        ...
        const $targetsResultCount = $(dapfforwc_FILTER.resultCountSelector);
        ...
        if (dapfforwc_options["update_filter_options"] === "on" && $updatedForm.length) {
            ...
            const $form = $(dapfforwc_FILTER.formSelector);
            const $existingChildren = $form.children().slice(3);
            destroySelect2InScope($existingChildren);
        ...
        setExplicitOrderby($(dapfforwc_FILTER.formSelector), explicitOrderby);
        syncHiddenRefererField($(dapfforwc_FILTER.formSelector), window.location.href);
    ...
    function syncCollapseState($title, $items, isCollapsed) {
        if (isCollapsed) {
            collapseItems($items);
        } else {
            expandItems($items);
        ...
        $title.find(".rotatable").toggleClass("rotated", !isCollapsed);
        $title.attr("data-collapse-state", isCollapsed ? "collapsed" : "expanded");
    }

    function collapseAllSections(context) {
        const $ctx = context ? $(context) : $(document);
        $ctx.find(COLLAPSE_SELECTORS.group).each(function () {
            const $group = $(this);
            const $title = $group.find(COLLAPSE_SELECTORS.title).first();
            const $items = $group.children().not(COLLAPSE_SELECTORS.title);
            syncCollapseState($title, $items, true);
        });
    }
    ...
    function restoreCollapseState() {
        $(COLLAPSE_SELECTORS.group).each(function (index) {
            ...
            if (savedState) {
                syncCollapseState($title, $items, savedState.isCollapsed);
            }
        });
    }
    ...
    function buildCachedHtmlWithFormState(fullUrl) {
        try {
            ...
            const orderbyParamKeys = [attrprefix.orderby, 'orderby'].filter(k => k && k !== '');
            const skipKeys = new Set(['gm_pf_fragment', 'per_page', 'cat_operator', 'tag_operator', 'terms_operator', 'brand_operator', 'order', 'default_min', 'default_max', ...paginationParamKeys, ...orderbyParamKeys]);
            ...
            form.querySelectorAll('input[name="_wp_http_referer"]').forEach(el => {
                el.setAttribute('value', toRelativeFilterUrl(urlObj.toString()));
            });
 ...
 */
    function syncFormStateFromUrl($form, url) {
        try {
            ...
            const orderbyParamKeysSet = new Set([attrprefix.orderby, 'orderby'].filter(k => k && k !== ''));

            // Reset all inputs first
            $form.find('input[type="checkbox"], input[type="radio"]').prop('checked', false);
            $form.find('select option').prop('selected', false);
            syncHiddenRefererField($form, url);
            ...
            for (const [key, value] of params.entries()) {
                ...
                if (paginationParamKeysSet.has(key) || orderbyParamKeysSet.has(key) || ['gm_pf_fragment', 'per_page', 'cat_operator', 'tag_operator', 'terms_operator', 'brand_operator', 'order'].includes(key)) {
                    continue;
                }
    ...
    function initSorting() {
        if (!window.dapfforwcSortingCaptureAttached) {
            window.dapfforwcSortingCaptureAttached = true;
            document.addEventListener('change', function (event) {
                const target = event && event.target;
                if (!target || typeof target.matches !== 'function') {
                    return;
                }
                if (!target.matches(dapfforwc_FILTER.sortingSelector)) {
                    return;
                }
                ...
                onsortchange($(target).val());
            }, true);
        }

        // Prevent native form submission for any form containing the configured sorting control.
        $(document)
            .off('submit.dapfforwcSorting', 'form')
            .on('submit.dapfforwcSorting', 'form', function (event) {
                const $form = $(this);
                let hasSortingControl = false;
                try {
                    hasSortingControl = $form.is(dapfforwc_FILTER.sortingSelector) || $form.find(dapfforwc_FILTER.sortingSelector).length > 0;
                } catch (_error) {
                    hasSortingControl = false;
                }
                if (hasSortingControl) {
                    event.preventDefault();
                }
            });

        $(document)
            .off('change.dapfforwcSortingAstra', '.ast-woocommerce-shop-page-modern-style')
            .on('change.dapfforwcSortingAstra', '.ast-woocommerce-shop-page-modern-style', function (e) {
                const $target = $(e.target);
                if (!$target.is(dapfforwc_FILTER.sortingSelector)) {
                    return;
                }
                e.preventDefault();
                e.stopImmediatePropagation();
                const sortValue = $target.val();
                onsortchange(sortValue);
            });

        $(document)
            .off('change.dapfforwcSortingSelect', dapfforwc_FILTER.sortingSelector)
            .on('change.dapfforwcSortingSelect', dapfforwc_FILTER.sortingSelector, function (e) {
                e.preventDefault();
                e.stopImmediatePropagation();
    ...
    function onsortchange(sortValue) {
        ...
        setExplicitOrderby($form, sortValue);

        // Reset to page 1 when sorting changes
        let $pageInput = $form.find('#filter-page-num');
        if ($pageInput.length) {
            $pageInput.val(1);
        }
    ...
    function changePseudoElementContent(minValue, maxValue, activeHandle = null) {
        $(dapfforwc_FILTER.formSelector).find('.plugrogress-percentage').addClass('dapfforwcpro-price-labels-ready');
    ...
    function initPriceRangeInputs() {
        ...
        function syncUI(minVal, maxVal, activeHandle = null) {
            ...
            changePseudoElementContent(String(minVal), String(maxVal), activeHandle);
            $rangeInputs.eq(0).attr('aria-valuetext', String(minVal));
            $rangeInputs.eq(1).attr('aria-valuetext', String(maxVal));
        }
        ...
        $slider.off('click.pf').on('click.pf', function (event) {
            const rect = this.getBoundingClientRect();
            if (!rect.width) return;

            const ratio = clamp((event.clientX - rect.left) / rect.width, 0, 1);
            const clickedValue = Math.round(minPriceDefault + (ratio * (maxPriceDefault - minPriceDefault)));
            const currentMin = Math.floor(parseFloat($rangeInputs.eq(0).val()) || minPriceDefault);
            const currentMax = Math.ceil(parseFloat($rangeInputs.eq(1).val()) || maxPriceDefault);
            ...
            if (clickedValue <= currentMin) {
                nextMin = clickedValue;
                targetIndex = 0;
            } else if (clickedValue >= currentMax) {
                nextMax = clickedValue;
                targetIndex = 1;
            } else if (Math.abs(clickedValue - currentMin) <= Math.abs(clickedValue - currentMax)) {
                nextMin = clickedValue;
                targetIndex = 0;
            } else {
                nextMax = clickedValue;
            ...
            syncUI(nextMin, nextMax, targetIndex === 0 ? 'min' : 'max');
            $rangeInputs.eq(targetIndex).trigger('change');
        });
    ...
    function applyFiltersFromUrl(filtersString) {
        ...
        if (!useNewFormat) {
            ...
            if (skuFromUrl) getFilterField($form, 'sku').val(skuFromUrl);
            const discountFromUrl = urlParams.get(attrprefix.discount ?? 'discount') || urlParams.get('discount');
            if (discountFromUrl) getFilterField($form, 'discount').val(discountFromUrl);
            const dateFromUrl = urlParams.get(attrprefix.date_filter ?? 'date') || urlParams.get('date_filter');
            if (dateFromUrl) getFilterField($form, 'date_filter').val(dateFromUrl);
        ...
        for (const [key, value] of urlParams.entries()) {
            if (key === 'filters' || key === orderbyPrefix || key === 'orderby' || key === paginationPrefix || key === 'paged' || key === 'product-page' || key === 'page') continue; // Skip the format flag, sorting, and pagination state
                ...
                values.forEach(val => {
                    ...
                    if (!$inputs.length && !$options.length) {
                        if (attributeName === 'price' && val.includes('-')) {
                            ...
                            getFilterField($form, 'sku').val(val).trigger('change');
                        } else if (attributeName === 'discount') {
                            getFilterField($form, 'discount').val(val).trigger('change');
                        } else if (attributeName === 'date_filter') {
                            getFilterField($form, 'date_filter').val(val).trigger('change');
                        }
                    }
                });
            }
    ...
    function transformToSeoUrl(url, seoOptions) {
        ...
        const orderbyKey = (prefixes && prefixes.orderby && prefixes.orderby !== '') ? prefixes.orderby : "orderby";
        ...
        const normalizedOrderbyKey = orderbyKey && orderbyKey !== "" ? orderbyKey : "orderby";
        const orderbyRaw = (searchParams.get(normalizedOrderbyKey) || searchParams.get('orderby') || '').trim();
        if (orderbyRaw && orderbyRaw !== 'menu_order') {
            seoParams.append(normalizedOrderbyKey, orderbyRaw);
        }
        ...
        const hasSeoFilterState = Array.from(seoParams.keys()).some(function (key) {
            return key !== normalizedPaginationKey && key !== normalizedOrderbyKey;
        });
        const orderedSeoEntries = Array.from(seoParams.entries()).filter(function ([key]) {
            return key !== 'filters';
        });
        if (hasSeoFilterState) {
            orderedSeoEntries.unshift(['filters', '1']);
        }

        // Build the new URL
        const baseUrl = urlObj.origin + urlObj.pathname;
        const seoQueryString = new URLSearchParams(orderedSeoEntries).toString();
    ...
    function destroySelect2Instance($select) {
        if (!$select || !$select.length) return;

        $select.off('.dapfforwcproSelect2');
        $select.removeData('dapfforwcproPreventSelect2Open');

        if ($select.data('select2')) {
            try {
                $select.select2('close');
            } catch (_error) {
                // Ignore stale Select2 close errors during AJAX refreshes.
            }

            try {
                $select.select2('destroy');
            } catch (_error) {
                // Ignore stale Select2 destroy errors during AJAX refreshes.
            }
        }
    ...
    function destroySelect2InScope($scope) {
        if (!$.fn || typeof $.fn.select2 !== 'function') return;

        const $root = ($scope && $scope.length) ? $scope : $(dapfforwc_FILTER.formSelector);
        $root.find('select.select2-hidden-accessible, .pluginy_select2.filter-select, select.select2_classic').each(function () {
            destroySelect2Instance($(this));
        });
    }

    function closeSelect2InScope($scope) {
        if (!$.fn || typeof $.fn.select2 !== 'function') return;
    ...
    function bindSelect2GuardEvents($select) {
        $select
            .off('.dapfforwcproSelect2')
            .on('select2:clearing.dapfforwcproSelect2 select2:unselecting.dapfforwcproSelect2', function () {
                $(this).data('dapfforwcproPreventSelect2Open', true);
            })
            .on('select2:clear.dapfforwcproSelect2 select2:unselect.dapfforwcproSelect2', function () {
                const $currentForm = $(this).closest(dapfforwc_FILTER.formSelector);
                setTimeout(function () {
                    closeSelect2InScope($currentForm);
                }, 0);
            })
            .on('select2:opening.dapfforwcproSelect2', function (event) {
                if ($(this).data('dapfforwcproPreventSelect2Open')) {
                    $(this).removeData('dapfforwcproPreventSelect2Open');
                    event.preventDefault();
                }
            })
            .on('select2:close.dapfforwcproSelect2', function () {
                $(this).removeData('dapfforwcproPreventSelect2Open');
            });
    }

    function initializeSelect2Element($select) {
        if (!$select || !$select.length) return;

        if ($select.hasClass('select2-hidden-accessible') && $select.data('select2')) {
            bindSelect2GuardEvents($select);
            return;
        }

        destroySelect2Instance($select);

        $select.select2({
            placeholder: TEXTS.select2_placeholder,
            allowClear: true,
            width: '100%'
        });
    ...
    function initializeCollapsible() {
        ...
        $(COLLAPSE_SELECTORS.title).each(function () {
            ...
            syncCollapseState($title, $items, startCollapsed);
            if (startCollapsed) {
            ...
            $clickTarget.off("click.dapfforwcCollapse").on("click.dapfforwcCollapse", function (e) {
                ...
                if ($origin.closest('.search_terms, .tooltip-icon, .dapfforwcpro-tooltip-bubble').length) {
                    return;
                }
                e.stopPropagation();
                ...
                syncCollapseState($title, $items, !wasCollapsed);
            });
        });

        const $currentForm = getCurrentForm();
        if ($currentForm.length) {
            $currentForm.addClass("dapfforwc-collapse-ready");
        }
    ...
    $(document).on('click', '.show-sub-cata', function (event) {
        ...
        event.stopPropagation();
        const $btn = $(this);
        const $node = $btn.closest('.dapfforwcpro-hierarchy-node');

        // Try multiple strategies to find the child categories container
        let $childCategories = $node.children('.child-categories').first();
        if (!$childCategories.length) $childCategories = $btn.closest('.dapfforwcpro-category-row').next('.child-categories');
    ...
    const getTermSearchLabelText = function ($label) {
        if (!$label || !$label.length) {
            return '';
        }

        const optionTitle = $label.find('.option_title').first().text().trim();
        if (optionTitle !== '') {
        ...
        if (labelTitle !== '') {
            return labelTitle.toLowerCase();
        }

        const $input = $label.find('input').first();
        const inputTitle = (($input.attr('title') || '') + ' ' + ($input.attr('aria-label') || '')).trim();
        if (inputTitle !== '') {
            return inputTitle.toLowerCase();
        }

        const labelText = $label
            .clone()
            .find('input, svg, img, script, style')
            .remove()
            .end()
            .text()
            .replace(/\s+/g, ' ')
    ...
    const applyTermSearch = function ($input) {
        if (!$input || !$input.length) {
            return;
        }

        const searchValue = (($input.val() || '') + '').toLowerCase().trim();
        const $filterGroup = $input.closest('.plugincy-filter-group');
        const $labels = $filterGroup.find('.items label');

        $labels.each(function () {
            const $label = $(this);
            const labelText = getTermSearchLabelText($label);
            const shouldShow = searchValue === '' || labelText.indexOf(searchValue) !== -1;

            if (shouldShow) {
                if ($label.hasClass("filterout-label")) {
                    $label.removeClass("filterout-label");
                }
            } else if (!$label.hasClass("filterout-label")) {
                $label.addClass("filterout-label");
            }
        });

        // Hierarchical categories use toggle controls outside the label.
        // Hide/show at node level so orphan "+" controls do not remain visible.
        const $hierarchyNodes = $filterGroup.find('.items .dapfforwcpro-hierarchy-node');
        if (!$hierarchyNodes.length) {
            return;
        }

        if (searchValue === '') {
            $hierarchyNodes.removeClass('filterout-node');
            $filterGroup.find('.show-sub-cata').removeClass('filterout-toggle');

            // Restore only branches that were expanded by search.
            $hierarchyNodes.each(function () {
                const $node = $(this);
                const $childContainer = $node.children('.child-categories').first();
                if ($childContainer.length && $childContainer.hasClass('dapfforwc-search-opened')) {
                    $childContainer.hide().removeClass('dapfforwc-search-opened');
                    const $toggle = $node.children('.dapfforwcpro-category-row').find('.show-sub-cata').first();
                    if ($toggle.length) {
        ...
        $($hierarchyNodes.get().reverse()).each(function () {
            const $node = $(this);
            const $row = $node.children('.dapfforwcpro-category-row').first();
            const $rowLabel = $row.find('label').first();
            let rowVisible = $rowLabel.length ? !$rowLabel.hasClass('filterout-label') : false;

            const $childContainer = $node.children('.child-categories').first();
            const $directChildNodes = $childContainer.length ? $childContainer.children('.dapfforwcpro-hierarchy-node') : $();
            const hasVisibleChild = $directChildNodes.filter(function () {
                return !$(this).hasClass('filterout-node');
            }).length > 0;

            // Keep parent context visible when a descendant matches, so we don't
            // render a standalone +/- toggle row without its category label.
            if (!rowVisible && hasVisibleChild && $rowLabel.length) {
                $rowLabel.removeClass('filterout-label');
                rowVisible = true;
            }

            const shouldShowNode = rowVisible || hasVisibleChild;
            $node.toggleClass('filterout-node', !shouldShowNode);

            const $toggle = $row.find('.show-sub-cata').first();
            if ($toggle.length) {
                const showToggle = shouldShowNode && hasVisibleChild;
                $toggle.toggleClass('filterout-toggle', !showToggle);

                if (showToggle && $childContainer.length && $childContainer.css('display') === 'none') {
                    $childContainer.show().addClass('dapfforwc-search-opened');
    ...
    $(document).on('input keyup', '.search-container input.search-terms', function () {
        applyTermSearch($(this));
    });

    $(document).on('click', '.plugincy-term-search-submit', function (event) {
        event.preventDefault();
        applyTermSearch($(this).closest('.search-container').find('input.search-terms').first());
    });
