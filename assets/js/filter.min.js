...
(function ($) {
    ...
    function collectPaginationSelectors() {
        ...
        LOAD_MORE_WRAPPER_HINTS.forEach(sel => addSelector(sel, true));
    ...
    function dispatchFilteredPaginationRequest(payload, interactionOptions) {
        if (!payload) return;
        console.log('Pagination Request Payload:', payload);
    ...
    function gmPfWakeLazyImgs(rootEl) {
        var root = rootEl || document;
        var imgs = root.querySelectorAll('img.gm-pf-lazy[data-src]');
        console.log('Lazy Images to Wake:', imgs);
    ...
    function renderAjaxHTML(response, currentUrl, url) {
        ...
        paginationTargetsList.forEach(function (selector) {
            ...
            const defaultHtml = $sources.length ? $sources.eq(0).html() : '';
            $targets.each(function (i) {
                const $target = $(this);
                const htmlContent = $sources.length > 1 ? $sources.eq(i).html() : defaultHtml;
                $target.html(htmlContent);
                const hasContent = hasRenderableContent(htmlContent);
                $target.toggle(hasContent);
                if (hasContent) {
                    $target.removeClass('dapfforwc-pagination-empty').removeAttr('aria-hidden');
                } else {
                    $target.addClass('dapfforwc-pagination-empty').attr('aria-hidden', 'true');
                }
            });
        });
