jQuery(document).ready(function ($) {
    ...
    function updateUrlFilters() {
        if (!dapfforwc_seo_permalinks_options || dapfforwc_seo_permalinks_options.use_attribute_type_in_permalinks !== "on") {
            ...
            if (dapfforwc_options.default_filters) {
                ...
                defaultFilters = [
                    ...
                    ...(defaultFilters["tag[]"] || []),
                    ...(Object.values(defaultFilters.attribute || {}).flat())
                ];
                let urlvalues = currentPage.split('/');
        ...
        // Build a reverse mapping from prefix to attribute name
        const prefixToAttribute = {};
        if (prefixOptions && prefixOptions.attribute) {
            for (const attrName in prefixOptions.attribute) {
                const prefix = prefixOptions.attribute[attrName];
                if (prefix) {
                    prefixToAttribute[prefix] = attrName;
                }
            }
        }
        ...
        $('#product-filter input:checked, .rfilterbuttons input:checked').each(function () {
            ...
            // Determine filter type based on input name
            ...
            } else if (name && name.includes('attribute')) {
                // Extract attribute name from the input name
                const attrMatch = name.match(/attribute\[(.*?)\]/);
                if (attrMatch && attrMatch[1]) {
                    const attrName = attrMatch[1];
                    filterType = prefixOptions.attribute && prefixOptions.attribute[attrName] ?
                        prefixOptions.attribute[attrName] : attrName;
                } else {
                    filterType = 'attr'; // Default
                }
            } else {
                filterType = 'attr'; // Default
            }
        ...
        $('#product-filter select').each(function () {
            ...
            if (selectName && selectName.includes('category')) {
                ...
                // Extract attribute name if this is an attribute select
                const attrMatch = selectName && selectName.match(/attribute\[(.*?)\]/);
                if (attrMatch && attrMatch[1]) {
                    const attrName = attrMatch[1];
                    filterType = prefixOptions.attribute && prefixOptions.attribute[attrName] ?
                        prefixOptions.attribute[attrName] : attrName;
        ...
        if (dapfforwc_options.default_filters && dapfforwc_options.default_filters[currentPage]) {
            const defaultFilters = dapfforwc_options.default_filters[currentPage];

            // Handle category default filters
            const categoryPrefix = prefixOptions.category || 'cata';
            if (filtersByType[categoryPrefix] && defaultFilters['category[]']) {
                filtersByType[categoryPrefix] = filtersByType[categoryPrefix].filter(value =>
                    !defaultFilters['category[]'].includes(value));

                // Remove empty arrays
                if (filtersByType[categoryPrefix].length === 0) {
                    delete filtersByType[categoryPrefix];
                }
            }

            // Handle tag default filters
            const tagPrefix = prefixOptions.tag || 'tag';
            if (filtersByType[tagPrefix] && defaultFilters['tag[]']) {
                filtersByType[tagPrefix] = filtersByType[tagPrefix].filter(value =>
                    !defaultFilters['tag[]'].includes(value));

                // Remove empty arrays
                if (filtersByType[tagPrefix].length === 0) {
                    delete filtersByType[tagPrefix];
                }
            }

            // Handle attribute default filters
            if (defaultFilters.attribute) {
                // For each attribute in the default filters
                for (const attrName in defaultFilters.attribute) {
                    // Find the prefix used for this attribute
                    const attrPrefix = prefixOptions.attribute && prefixOptions.attribute[attrName]
                        ? prefixOptions.attribute[attrName]
                        : attrName;

                    // If we have values selected for this attribute
                    if (filtersByType[attrPrefix]) {
                        // Filter out the default values
                        filtersByType[attrPrefix] = filtersByType[attrPrefix].filter(value =>
                            !defaultFilters.attribute[attrName].includes(value));

                        // Remove empty arrays
                        if (filtersByType[attrPrefix].length === 0) {
                            delete filtersByType[attrPrefix];
                        }
                    }
                }
            }
